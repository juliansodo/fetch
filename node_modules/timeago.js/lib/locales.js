...
var ES = 'segundo_minuto_hora_dia_semana_mes_año'.split('_');
...
var en_US = function en_US(number, index) {
  ...
  var unit = ES[parseInt(index / 2)];
  if (number > 1) unit += 's';
  return ["".concat(number, " ").concat(unit, " ago"), "in ".concat(number, " ").concat(unit)];
};

var es_AR = function es_AR(number, index) {
  if (index === 0) return ['justo ahora', 'ahora mismo'];
  var unit = ES[parseInt(index / 2)];
  if (number > 1) unit += 's';
  return ["hace ".concat(number, " ").concat(unit, ""), "in ".concat(number, " ").concat(unit)];
};
/**
 * 所有的语言
